...
def text_to_tok(text, tokenizer):
    return torch.unsqueeze(torch.tensor(tokenizer.encode(text)), dim = 0)

def tok_to_text(toks, tokenizer):
    toks = torch.squeeze(toks,dim=0).tolist()
    return tokenizer.decode(toks)

def generate_text(inp, model, device, max_gen_tokens, temperature = 1.0, top_k = None, context_length = None):
    inp = inp.to(device)
    model.to(device)
    model.eval();
